...
try {
   if (!isset($pdo) || !$pdo instanceof PDO) {
    ...
    // Récupération des données de l'emprunt principal
    ...
    // Variables pour stocker les informations de journal et contrepartie
    $journalLie = "N/A";
...
<div class="container mt-5">
    ...
    <?php else: ?>
        <div class="details-container">
            ...
            <p><strong>TEG:</strong> <?= isset($emprunt['Taux_Effectif_Global']) ? number_format($emprunt['Taux_Effectif_Global'] * 1, 4, ',', ' ') . '%' : 'N/A' ?></p>
            ...
            <p><strong>Intérêt SP Taux:</strong> <?= isset($emprunt['Interet_SP_Taux']) ? number_format($emprunt['Taux_Effectif_Global'] * 1, 4, ',', ' ') . '%' : 'N/A' ?></p>
            ...
            ---
            <h3>Décomposition des Écritures Comptables par Échéance</h3>
                    ...
                    <table class="table table-striped table-bordered">
                        <thead>
                            <tr>
                                ...
                                <th>Libellé</th>
                                <th>Compte Débité</th>
                                <th>Compte Cr&eacutedit&eacute</th>
                        ...
                        <tbody>
                            <?php
                            // Assuming $journalLie and $contrepartieLiee are defined
                            // e.g., $journalLie = "BQ"; $contrepartieLiee = "512000"; // Bank account
                            // You might fetch these from your database or configuration
                            $journalLie = $journalLie ?? 'N/A'; // Default if not set
                            $contrepartieCompte = $contrepartieLiee ?? '512000'; // Default Bank Account if not set
                            ...
                            foreach ($echeances as $echeance):
                                ...
                                // Sum up all fees and taxes for the base amount of TVA
                                ...
                                // Calculate TVA on Interest SP and other fees/taxes (if applicable)
                                $TaxesTVA = ($interetSP + $totalFeesTaxes) * $tvaRate;

                                // Total amount paid for this installment
                                $montantTotalEcheance = $amortissement + $interetSP + $totalFeesTaxes + $TaxesTVA;

                                // Account for loan principal (liability decreases, so it's a debit)
                                if ($amortissement > 0):
                            ?>
                                <tr>
                                    <td><?= $dateEcheanceFormatted ?></td>
                                    <td>Remboursement Capital Emprunt Échéance <?= $numeroEcheance ?></td>
                                    <td>164000</td> <td><?= $contrepartieCompte ?></td> <td class="text-right"><?= number_format($amortissement, 2, ',', ' ') ?></td>
                                    <td class="text-right">0,00</td>
                                </tr>
                            <?php
                                endif;

                                // Account for Interest SP (expense, so it's a debit)
                                if ($interetSP > 0):
                            ?>
                                <tr>
                                    <td><?= $dateEcheanceFormatted ?></td>
                                    <td>Charges d'Intérêt (Intérêt SP) Échéance <?= $numeroEcheance ?></td>
                                    <td>661000</td> <td><?= $contrepartieCompte ?></td> <td class="text-right"><?= number_format($interetSP, 2, ',', ' ') ?></td>
                                    <td class="text-right">0,00</td>
                                </tr>
                            <?php
                                endif;

                                // Account for other Fees/Taxes (expense, so it's a debit)
                                if ($totalFeesTaxes > 0):
                            ?>
                                <tr>
                                    <td><?= $dateEcheanceFormatted ?></td>
                                    <td>Frais et Taxes bancaires Échéance <?= $numeroEcheance ?></td>
                                    <td>627000</td> <td><?= $contrepartieCompte ?></td> <td class="text-right"><?= number_format($totalFeesTaxes, 2, ',', ' ') ?></td>
                                    <td class="text-right">0,00</td>
                                </tr>
                            <?php
                                endif;

                                // Account for TVA on Interest and Fees (Input TVA, so it's a debit)
                                if ($TaxesTVA > 0):
                            ?>
                                <tr style="border-bottom: 2px solid #ddd;">
                                    <td><?= $dateEcheanceFormatted ?></td>
                                    <td>TVA D&eacute;ductible sur Int&eacute;r&ecirc;ts et Frais Échéance <?= $numeroEcheance ?></td>
                                    <td>445660</td> <td><?= $contrepartieCompte ?></td> <td class="text-right"><?= number_format($TaxesTVA, 2, ',', ' ') ?></td>
                                    <td class="text-right">0,00</td>
                                </tr>
                            <?php
                                endif;

                                // The total credit entry for the bank account for this installment
                                // This assumes all the above debit entries correspond to a single credit from the bank.
                                // This row is the 'contrepartie' for all the debit entries above.
                            ?>
                                <tr class="info"> <td><?= $dateEcheanceFormatted ?></td>
                                    <td>Paiement total Échéance <?= $numeroEcheance ?></td>
                                    <td></td> <td><?= $contrepartieCompte ?></td> <td class="text-right">0,00</td>
                                    <td class="text-right"><?= number_format($montantTotalEcheance, 2, ',', ' ') ?></td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
