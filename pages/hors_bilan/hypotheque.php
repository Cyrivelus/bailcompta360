...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'supprimer') {
        ...
        if (supprimerHypotheque($pdo, $id_hypotheque_a_supprimer)) {
            ...
            throw new Exception("Échec de la suppression de l'hypothèque. Elle peut avoir des paiements associés.");
        }
    }
...
<div class="container-fluid mt-4">
    ...
    <table class="table table-striped table-bordered">
        ...
        <tbody>
            <?php if (!empty($hypotheques)): ?>
                <?php foreach ($hypotheques as $hypotheque): ?>
                    <tr>
                        ...
                        <td><?= number_format($hypotheque['montant_pret'], 2, ',', ' '); ?> €</td>
                        <td><?= htmlspecialchars($hypotheque['taux_interet']); ?> %</td>
                        <td><?= htmlspecialchars($hypotheque['duree_annees']); ?></td>
            ...
            <?php else: ?>
                <tr>
                    <td colspan="7" class="text-center">Aucune hypothèque trouvée.</td>
                </tr>
            <?php endif; ?>
        </tbody>
    </table>
